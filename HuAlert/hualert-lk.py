...
#social video fast forward
def social():
    counter = 0
    while(1):
        counter += 1
        ret, frame = social_cap.read()
        if counter%6 == 0: #skip frames to fast forward
            if(ret):
                frame = cv2.resize(frame, (width, height))
                frame = cv2.GaussianBlur(frame, (1,11), 0,0) #blur video
                show(frame)
                k = cv2.waitKey(2)
                if k == 27:
                    break
...
social_cap = cv2.VideoCapture('social.mp4')
